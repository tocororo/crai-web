/* ============================================================
   UNIVERSIDAD DEL FUTURO - Main JavaScript
   ============================================================ */

/* --- Carrusel (accesible desde noticias.js) --- */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* --- Local pages index --- */
  const LOCAL_PAGES = [
    { title: 'Inicio', url: '../index.html', keywords: 'inicio, bienvenidos, crai' },
    { title: 'La Universidad', url: 'la-universidad.html', keywords: 'universidad, historia, mision, vision, autoridad' },
    { title: 'Estudios - Oferta Académica', url: 'estudios.html', keywords: 'estudios, programas, carreras, grado, posgrado, curso' },
    { title: 'Admisión', url: 'admision.html', keywords: 'admision, inscripcion, requisitos, becas, matricula' },
    { title: 'Investigación', url: 'investigacion.html', keywords: 'investigacion, proyectos, publicaciones, ciencia' },
    { title: 'Vida en el Campus', url: 'vida-campus.html', keywords: 'vida, campus, deportes, cultura, residencia' },
    { title: 'Actualidad', url: 'actualidad.html', keywords: 'noticias, eventos, actualidad, agenda' },
    { title: 'Portal de Transparencia', url: 'transparencia.html', keywords: 'transparencia, informes, datos, presupuesto' },
  ];

  const RC_API = '/api/rc-search';
  ...
  function updateIconAndClear() {
    if (searchIcon) {
      searchIcon.classList.toggle('has-text', searchInput.value.length > 0);
    }
    if (searchClear) {
  ...
  async function executeSearch(query) {
    ...
    const [localResults, rcResults] = await Promise.all([
      searchLocal(query),
      searchRC(query, searchAbort.signal),
    ]);

    renderAllResults(query, localResults, rcResults);
  }

  function searchLocal(query) {
    const q = query.toLowerCase();
    return LOCAL_PAGES.filter(p =>
      p.title.toLowerCase().includes(q) ||
      p.keywords.toLowerCase().includes(q)
    ).map(p => ({ ...p, type: 'page' }));
  }

  async function searchRC(query, signal) {
    const endpoints = [
      `/api/rc-search?query=${encodeURIComponent(query)}`,
      `/api/rc-simple?query=${encodeURIComponent(query)}`,
    ];
  ...
  function parseRCXML(xmlText) {
    try {
      ...
      return Array.from(items).slice(0, 5).map(item => ({
        title: item.querySelector('name')?.textContent?.trim() || 'Sin título',
        description: '',
        url: 'https://rc.upr.edu.cu' + (item.querySelector('link')?.textContent?.trim() || ''),
        type: 'publication',
        authors: item.querySelector('handle')?.textContent?.trim() || '',
        date: item.querySelector('lastModified')?.textContent?.trim()?.split(' ')[0] || '',
      }));
    } catch {
      return null;
    }
  ...
  function renderAllResults(query, local, rc) {
    const hasLocal = local && local.length > 0;
    const hasRC = rc && rc.length > 0;
    ...
    if (hasRC) {
      html += `<div class="search-results-group">`;
      html += `<div class="search-results-group-header"><i class="fas fa-book-open"></i> Repositorio Científico (RC)</div>`;
      rc.forEach(p => {
        currentItems.push({ url: p.url, type: 'publication' });
        html += `
          <div class="search-results-item" data-index="${currentItems.length - 1}">
            <div class="search-results-item-icon publication"><i class="fas fa-file-alt"></i></div>
            <div class="search-results-item-content">
              <h4>${escapeHtml(p.title)}</h4>
              <p>${p.authors ? escapeHtml(p.authors) : escapeHtml(truncateText(p.description, 80))}</p>
            </div>
            <span class="search-results-item-badge api">RC</span>
          </div>`;
      });
      html += `</div>`;
    }

    if (!hasRC && isFileProtocol && hasLocal) {
      html += `<div class="search-results-group">`;
      html += `<div class="search-results-group-header" style="opacity:0.4;"><i class="fas fa-book-open"></i> Repositorio Científico</div>`;
      html += `<div class="search-results-empty" style="padding:16px;"><p style="font-size:0.8rem;">Conéctate a la red UPR para buscar publicaciones</p></div>`;
      html += `</div>`;
    }
